...
class SimpleBacktester:
    ...
    def run(self):
        ...
        for i in range(self.window_size, len(self.data)):
            ...
            if signal_strength is not None and signal_strength > 0:
                ...
                if cost <= self.cash:
                    # Get dynamic stop loss and take profit levels
    ...
    def check_stops(self, data):
        # Check if we need to close any positions based on stop losses
        positions_to_close = []
        ...
        for position in positions_to_close:
            self._close_position(position, data, 'stop_loss')
...
def main():
    ...
    X_val_tensor = torch.FloatTensor(X_val_seq)  # Shape: (batch, seq_len, features)
    ...
    cnn_acc = evaluate_model(cnn_model, X_val_tensor_cnn, y_val_tensor)
